...
const registry_1 = require("./fileSelectors/registry");
...
function createNode(name, config) {
    ...
    const node = {
        ...
        agentMode: typeof config.agent_mode === "string"
            ? config.agent_mode
            : typeof config.agentMode === "string"
                ? config.agentMode
                : undefined,
        answerMode: typeof config.answer_mode === "string"
            ? config.answer_mode
            : typeof config.answerMode === "string"
                ? config.answerMode
                : undefined,
        fileSelectMode: typeof config.file_select_mode === "string"
...
function isRegionModelUnavailable(message) {
    const lower = message.toLowerCase();
    return (lower.includes("model not available") ||
        lower.includes("doesn't serve your region") ||
        lower.includes("does not serve your region"));
}
function buildAgentPrompt(skillPrompt, selectedFiles) {
    const parts = [skillPrompt.trim()];
...
async function showSkillWindow(node, extensionPath) {
    ...
    const fileSelector = (0, registry_1.getFileSelector)(fileSelectMode);
    const fileSelectorState = { selectedFiles: [] };
    const panel = vscode.window.createWebviewPanel("skillsBankSkillPreview", `SkillsBank - ${node.name}`, vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
...
<body>
  ...
  <div class="file">模型配置：${escapeHtml(node.agentMode?.trim() || "未配置（不传 --model）")}</div>
  <div class="file">回答模式（answer_mode）：${escapeHtml(node.answerMode?.trim() || "agent")}</div>
